/**
 * pixel-snow.js — Vanilla Three.js port of <PixelSnow /> from React Bits
 * Custom GLSL ShaderMaterial rendering retro pixelated falling snow/particles.
 */
...
function initPixelSnow(container, opts = {}) {
  const {
    color           = '#b45309', // warm amber accent color matching store theme
    flakeSize       = 0.01,
    minFlakeSize    = 1.25,
    pixelResolution = 200,
    speed           = 1.25,
...
// Auto-initialize PixelSnow on container #pixel-snow-canvas-container
document.addEventListener('DOMContentLoaded', () => {
  const container = document.getElementById('pixel-snow-canvas-container');
  if (!container) return;

  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

  initPixelSnow(container, {
    color: '#d97706',
    flakeSize: 0.012,
    minFlakeSize: 1.5,
    pixelResolution: 220,
