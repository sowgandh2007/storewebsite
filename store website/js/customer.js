// customer.js - Storefront Catalog & Shopping Cart Controller (Storefront Dark Theme Version)
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderCategories() {
        ...
        // Rebuild desktop tag links
        categoriesContainer.innerHTML = "";
        categories.forEach(cat => {
            const btn = document.createElement("button");
            btn.className = `px-4 py-2 text-xs font-bold rounded-xl transition-all ${
                activeCategory === cat 
                ? "bg-brand-600 text-white shadow-md shadow-brand-500/20 border border-brand-500/30" 
        ...
        if (categoryDropdown) {
            ...
            categories.forEach(cat => {
                ...
                btn.className = `w-full text-left px-4 py-2.5 text-xs font-semibold hover:bg-dark-800 transition-colors ${
                    activeCategory === cat ? "text-brand-500 font-bold bg-dark-800" : "text-slate-400"
                }`;
                btn.textContent = cat === "all" ? "All Products" : cat;
                btn.addEventListener("click", () => {
                    selectCategory(cat);
    ...
    function renderProducts() {
        ...
        filtered.forEach(p => {
            ...
            card.className = "bg-dark-900 rounded-2xl overflow-hidden border border-slate-850 shadow-md hover-lift transition-all duration-300 flex flex-col group animate-fade-in-up";
            ...
            // Cart Quantity controls on card
            ...
            if (p.available) {
                if (qtyInCart > 0) {
                    actionButtonHTML = `
                        <div class="flex items-center justify-between bg-dark-950 border border-slate-850 rounded-xl p-1">
                            <button onclick="changeCartQty('${p.id}', -1)" class="w-8.5 h-8.5 rounded-lg bg-dark-800 hover:bg-dark-750 text-white font-bold flex items-center justify-center transition-all shadow-sm border border-slate-800">
                    ...
                    actionButtonHTML = `
                        <button onclick="changeCartQty('${p.id}', 1)" class="w-full bg-brand-600 hover:bg-brand-700 active:bg-brand-850 text-white font-bold text-xs py-2.5 px-4 rounded-xl transition-all shadow-md flex items-center justify-center gap-1.5">
                ...
                <div class="p-5 flex-grow flex flex-col justify-between">
                    ...
                    <div>
                        <div class="flex items-baseline gap-2 mb-4 bg-dark-950 rounded-xl p-2.5 border border-slate-850">
                            <div>
                                <span class="block text-[9px] text-slate-500 font-extrabold uppercase leading-none">Our Price</span>
                                <span class="text-xl font-black text-white">${currency}${p.selling_price}</span>
                            </div>
                            ${savings > 0 ? `
                            <div class="border-l border-slate-800 pl-2.5 ml-1">
                                <span class="block text-[9px] text-slate-500 font-extrabold uppercase leading-none">MRP</span>
                                <span class="text-xs text-slate-500 line-through font-semibold">${currency}${p.market_price}</span>
                            </div>
                            ` : ""}
                        </div>
        ...
        if (cartKeys.length === 0) {
            cartItemsContainer.innerHTML = `
                <div class="flex flex-col items-center justify-center py-20 text-slate-500 gap-2 select-none text-center">
                    ...
                    <p class="text-[10px] text-slate-500 max-w-[200px]">Add products from our wholesale catalog to make a reservation request.</p>
                </div>
            `;
        ...
        cartKeys.forEach(productId => {
            ...
            cartRow.innerHTML = `
                <div class="flex-grow">
                    ...
                    <span class="text-[10px] text-slate-500 font-bold block mt-0.5">Category: ${product.category}</span>
    ...
    window.handleCartCheckout = async function(e) {
        ...
        try {
            // Write batch reservation record to database
    ...
    // Subscribe to live postgres table changes to re-sync
